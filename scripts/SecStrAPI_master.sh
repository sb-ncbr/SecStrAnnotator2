...
# # Download CIF files
# python3  $DOWNLOAD_DOMAINS  --format cif  --no_gzip  $DATA_DIR/cyps_all_$TODAY.simple.json  $STRUCTURES_DIR
...
#     $STRUCTURES_DIR  $TEMPLATE  $DATA_DIR/cyps_all_$TODAY.simple.json 

# Collect annotations and put them to SecStrAPI format
python3  $COLLECT_ANNOTATIONS  $DATA_DIR/cyps_all_$TODAY.json  $STRUCTURES_DIR  $ANNOTATIONS_DIR  --sequences_directory $SEQUENCES_DIR
python3  $COLLECT_ANNOTATIONS  $DATA_DIR/cyps_best_$TODAY.json  $STRUCTURES_DIR  $BEST_ANNOTATIONS_DIR  --sequences_directory $BEST_SEQUENCES_DIR
