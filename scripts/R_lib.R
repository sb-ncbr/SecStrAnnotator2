...
is.digit <- function(x) grepl("[[:digit:]]", x)

color_name = function(name) {
  name = as.character(name)
  n=nchar(name)
  if (is.digit(substr(name,1,1)))
    result = 'strand'
...
#helix_order = c("A'", "A", "B", "B''", "B'", "B'''", "C", "D", "E", "F", "F'", "G'", "G", "H", "I", "J", "J'", "K", "K'", "K'''", "K''", "L", "L'", "h")
helix_order = c("A'", "A", "B", "B'", "B''", "C", "D", "E", "F", "F'", "G'", "G", "H", "I", "J", "J'", "K", "K'", "K''", "L", "L'")
sheet_order = c("1a", "1b", "1c", "1d", "1e", "2a", "2b", "3a", "3b", "3c", "4a", "4b", "4c", "e")
#sheet_order_more = c("1z", "1a", "1b", "1c", "1d", "1e", "2a", "2b", "3a", "3b", "3c", "4a", "4b", "4c", "5a", "5b", "6a", "6b", "e")
#sheet_order_more = c("1a", "1b", "1c", "1d", "1e", "2a", "2b", "3a", "3b", "3c", "4a", "4b", "4c", "5a", "5b")
#sheet_order_more = c("1.1", "1.2", "1.3", "1.4", "1.5", "2.1", "2.2", "3.1", "3.2", "3.3", "4.1", "4.2", "4.3", "5.1", "5.2")
#sheet_order_more = c("1-1", "1-2", "1-3", "1-4", "1-5", "2-1", "2-2", "3-1", "3-2", "3-3", "4-1", "4-2", "4-3", "5-1", "5-2")
#sheet_order_more = c("0-1", "0-2", "1-0", "1-1", "1-2", "1-3", "1-4", "1-5", "2-1", "2-2", "3-1", "3-2", "3-3", "4-1", "4-2", "5-1", "5-2", "6-1", "6-2")
sheet_order_more = c("1-0", "1-1", "1-2", "1-3", "1-4", "1-5", "2-1", "2-2", "3-1", "3-2", "3-3", "4-1", "4-2", "5-1", "5-2", "6-1", "6-2")
sse_order = c("A'", "A", "1a", "1b", "B", "1e", "B''", "B'", "B'''", "C", "D", "3a", "E", "F", "F'", "G'", "G", "H", "I", "J", "J'", "K", "4c", "1d", "2a", "2b", "1c", "K'", "K'''", "K''", "L", "3c", "L'", "4a", "4b", "3b", "h", "e")
our_sses = c(helix_order, sheet_order_more)
...
only_sheets = function(df){
  df %>% filter(label %in% sheet_order_more)
}

order_x_labels_old = function(apostrophe_to_prime=TRUE) { 
  if (apostrophe_to_prime){
    scale_x_discrete(limits=c(helix_order, sheet_order_more), 
...
table_sse_lengths = function(df, ignore_zero=FALSE) {
  ...
  data = df %>% group_by(label) %>% summarise(avg_len=mean(length),sd_len=sd(length),type=color_name(label[1]))
}
...
jitterplot_sse_lengths = function(df, ignore_zero=FALSE) {
  df = df %>% mutate(type=mapply(color_name,label))
  if (ignore_zero) df = df %>% filter(length>0)
  ggplot(df) +
...
violinplot_sse = function(df, y_column, ignore_zero=FALSE, scale='width', width=0.9, group_col='type', title=NULL, stagger_labels=FALSE, turn_labels=FALSE) {
  ...
  ggplot(df) +
    aes(x=label, y=df[[y_column]], color=df[[group_col]]) + #color=type #  color=df[group_col]
...
boxplot_sse = function(df, y_column, ignore_zero=FALSE, scale='width', width=0.9, group_col='type', title='') {
  df = df %>% mutate(type=mapply(color_name,label))
  if (ignore_zero) df = df %>% filter(df$length>0)
  ggplot(df) +
...
violinplot_sse_lengths_multiset2 = function(..., set_names=NULL, ignore_zero=FALSE, scale='width', width=0.9, title=NULL, stagger_labels=FALSE, turn_labels=FALSE) {
  #width = max. width wrt. bar width
  #scale = 'width' (violins have equal width), 'area' (violins have equal area)
  #point is mean, line is median
...
table_sse_occurrence = function(df, alpha=0.05) {
  ...
  data = df %>% filter(length>0) %>% group_by(label) %>% count() %>% 
    mutate(freq=n/n_pdb, type=color_name(label), freq_min=agresti_coull_confidence_interval(n,n_pdb,alpha)[1], freq_max=agresti_coull_confidence_interval(n,n_pdb,alpha)[2])
  data
  }
...
plot_criterion = function(df, criterion, ignore_zero=FALSE, title='', y_label='Fulfilling criterion'){
  table = table_criterion(df, criterion, ignore_zero = ignore_zero) %>% group_by(label) %>% mutate(type=color_name(label))
  g = ggplot(table) + 
...
plot_criteria = function(df, ..., ignore_zero=FALSE, title='', y_label='Fulfilling criterion'){
  table = table_criteria(df, ..., ignore_zero = ignore_zero) %>% group_by(label, set) %>% mutate(type=color_name(label))
  g = ggplot(table) + 
...
plot_compare_sse_occurrence = function(..., set_names=NULL, show_confidence=TRUE, alpha=0.05, title=NULL, stagger_labels=FALSE, turn_labels=FALSE) {
  ...
  g = ggplot(t) +
    ...
    geom_col(position='dodge') + 
    geom_vline(xintercept=seq(1.5, length(unique(t$label))-0.5, 1), lwd=0.4, colour='gray90') +
...
two_sample_occurrence_prop.test = function(df1, df2, p_limit=0.05, print_all=FALSE, signif_digits=2) {
  n_pdb1 = n_distinct(df1$PDB)
  data1 = df1 %>% filter(length>0) %>% group_by(label) %>% count() %>% 
    mutate(present1 = n, absent1 = n_pdb1-n, freq1 = n/n_pdb1) %>% select(-n)
  n_pdb2 = n_distinct(df2$PDB)
