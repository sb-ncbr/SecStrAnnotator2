...
def perform_alignment(method, directory, template_id, template_chain, template_range, query_id, query_chain, query_range):
    ...
    if method == 'align':
        ...
        run_super(sel_t, sel_q, q)
        # cmd.super(sel_q, sel_t)
    elif method == 'cealign':
        run_cealign(sel_t, sel_q)
        # try:
        # 	run_cealign(sel_t, sel_q)
        # except CmdException:
        # 	print('WARNING: cealign failed, falling back to super.')
        # 	run_super(sel_t, sel_q, q)
    else:
...
def run_cealign(target, mobile):
    aln_result = cmd.cealign(target, mobile, object=ALIGNMENT_OBJECT)
    try:  # dev
        import json
...
def run_super(target, mobile, mobile_object):
    aln_tuple = cmd.super(mobile, target, object=ALIGNMENT_OBJECT)
    rmsd, n_atoms, n_cycles, rmsd1, aln_length, score, _ = aln_tuple
    rot_mat = cmd.get_object_matrix(mobile_object)
    aln_result = {'RMSD': rmsd, 'alignment_length': aln_length, 'rotation_matrix': rot_mat}
    try:  # dev
        import json
        aln_result[ALIGNED_RESIDUES] = aligned_residues_from_alignment_object(ALIGNMENT_OBJECT)
        with open(ALIGNMENT_FILE, 'w') as f:
